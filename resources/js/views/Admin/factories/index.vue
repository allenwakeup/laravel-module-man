<template>
    <div>
        <div class="admin_table_page_title">工厂列表</div>
        ...
        <div class="admin_table_list">
            <a-table
                ...
                <span slot="base" slot-scope="record" >
                    {{ (!!record.base && !$isEmpty(record.base.name)) ? record.base.name : '--' }}
                </span>
...
export default {
    ...
    data() {
      return {
          table: {
              actions: {
                  list: this.$api.moduleAdminFactories,
                  remove: this.$api.moduleAdminFactories
              },
              columns: [
                  {title:'#',dataIndex:'id',fixed:'left', width: 60},
                  {title:'生产基地',scopedSlots: { customRender: 'base' }},
          ...
          exporting:{ // 数据导出
              url: this.$api.moduleAdminFactories,
              columns: [
                  {
                      label: '编码',
                      name: 'code',
