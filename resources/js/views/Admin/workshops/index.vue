        ...
        <div class="admin_table_list">
            <a-table
                ...
                <span slot="factory" slot-scope="record" >
                    {{ (!!record.factory && !!record.factory.base && !$isEmpty(record.factory.base.name)) ? record.factory.base.name : '' }}
                    {{ (!!record.factory && !$isEmpty(record.factory.name)) ? record.factory.name : '' }}
                </span>
...
export default {
    ...
    data() {
      return {
          ...
          table: {
              actions: {
                  list: this.$api.moduleAdminWorkshops,
                  remove: this.$api.moduleAdminWorkshops
              },
              columns: [
                  {title:'#',dataIndex:'id',fixed:'left', width: 60},
                  {title:'工厂',scopedSlots: { customRender: 'factory' }},
          ...
          exporting:{ // 数据导出
              url: this.$api.moduleAdminWorkshops,
              columns: [
                  {
                      label: '编码',
                      name: 'code',
