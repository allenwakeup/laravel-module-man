<template>
    <div>
        <div class="admin_table_page_title">工序列表</div>
            ...
            <a-table
                ...
                <template slot="title" slot-scope="currentPageData">
                    ...
                    <div class="admin_table_handle_btn">
                        <a-button @click="$router.push('/Admin/goodcatch/m/man/processes/form')" type="primary" icon="plus">添加</a-button>
                ...
                <span slot="workshop" slot-scope="record" >
                    {{ (!!record.workshop && !!record.workshop.factory && !!record.workshop.factory.base && !$isEmpty(record.workshop.factory.base.name)) ? record.workshop.factory.base.name : '' }}
                    {{ (!!record.workshop.factory && !$isEmpty(record.workshop.factory.name)) ? record.workshop.factory.name : '' }}
                    {{ !!record.workshop ? record.workshop.name : '--' }}

                </span>


                <span slot="action" slot-scope="rows">
                    <a-button icon="edit" @click="$router.push('/Admin/goodcatch/m/man/processes/form/'+rows.id)">编辑</a-button>
                </span>
            </a-table>
...
export default {
    ...
    data() {
      return {
          table: {
              actions: {
                  list: this.$api.moduleAdminProcesses,
                  remove: this.$api.moduleAdminProcesses
              },
              columns: [
                  {title:'#',dataIndex:'id',fixed:'left', width: 60},
                  {title:'车间',scopedSlots: { customRender: 'workshop' }},
          ...
          exporting:{ // 数据导出
              url: this.$api.moduleAdminProcesses,
              columns: [
                  {
                      label: '编码',
                      name: 'code',
